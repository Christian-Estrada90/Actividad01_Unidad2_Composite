...
public class CarpetaArchivoController {
    ...
    public DefaultTreeModel mostrar() {
        DefaultMutableTreeNode noderoot = new DefaultMutableTreeNode(root.getNombre() + " - " + String.valueOf(root.getTamanio()) + "mb");
        ...
        for (var archivo : ((Carpeta) this.root).getListaArchivoComponent()) {
            ...
            if (archivo.getClass().getSimpleName().equals("Carpeta")) {
                ...
                if (((Carpeta) archivo).getListaArchivoComponent().isEmpty()) {
                    node = new DefaultMutableTreeNode(archivo.getNombre() + " - " + String.valueOf(archivo.getTamanio()) + "mb");
                } else {
                    node = obtenerNodo((Carpeta) archivo);
                }
            } else {
                node = new DefaultMutableTreeNode(archivo.getNombre() + "." + archivo.getClass().getSimpleName() + " - " + String.valueOf(archivo.getTamanio()) + "mb");
            }
            noderoot.add(node);
        }
        return model;
    ...
    private DefaultMutableTreeNode obtenerNodo(Carpeta carpeta) {
        DefaultMutableTreeNode nodeCarpeta = new DefaultMutableTreeNode(carpeta.getNombre() + " - " + String.valueOf(carpeta.getTamanio()) + "kb");
        for (var archivo : carpeta.getListaArchivoComponent()) {
            DefaultMutableTreeNode node;
            if (archivo.getClass().getSimpleName().equals("Carpeta")) {
                node = obtenerNodo((Carpeta) archivo);
            } else {
                node = new DefaultMutableTreeNode(archivo.getNombre() + "." + archivo.getClass().getSimpleName() + " - " + String.valueOf(archivo.getTamanio()) + "kb");
            }
